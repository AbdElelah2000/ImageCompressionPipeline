# -*- coding: utf-8 -*-
"""2x_downsample_train.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1lol13NYbhRDe9M1MzL1pSVIYirfEVe_j
"""
...
def pull_database():
  ...
  for image_in_database in tqdm(glob('/content/DIV2K_train_HR/*.png')):
    ...
    YUV_image = convert_RGB2YUV(image_train)
    Y_channel = YUV_image[:,:,0]
    Y_output = cv2.resize(Y_channel, (128, 128), interpolation = cv2.INTER_AREA)
    Y_input = cv2.resize(Y_output, None, fx=2, fy=2, interpolation = cv2.INTER_AREA)
...
error_callbk = tf.keras.callbacks.TensorBoard(log_dir='./drive/MyDrive/training/graph/2x_downsample/', histogram_freq=0, write_graph=True, write_images=True)
callbk_earlyStop = EarlyStopping(monitor='val_loss', patience=5, verbose=1, mode='min', restore_best_weights=True)


batch_size = 4
epochs = 100
train_model.fit(x_training, y_training, validation_data=(x_validation, y_validation), batch_size=batch_size, epochs=epochs, callbacks=[error_callbk, callbk_save_model, callbk_earlyStop])



train_model = load_model('drive/MyDrive/training/model/2x_downsample_model.h5')
img = cv2.imread("drive/MyDrive/training/test_image.png")
img_yuv = convert_RGB2YUV(img)
Y_input = img_yuv[:,:,0]
Y_pred = np.expand_dims(Y_input,axis=0)
y_downsampled = train_model.predict(Y_pred)

print("input:", img_yuv[:,:,0].shape)
print("downsampled:",y_downsampled.shape)

plt.subplot(211)
plt.imshow(img_yuv[:,:,0], cmap='gray')
plt.subplot(212)
plt.imshow(y_downsampled[0], cmap='gray')
plt.show()