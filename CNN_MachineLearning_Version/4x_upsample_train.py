# -*- coding: utf-8 -*-
"""4x_upsample_train.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1F4ugcoCClg_1HGvf59iq2thdEVRPcu78
"""
...
def pull_database():
  ...
  for img_dir in tqdm(glob('/content/DIV2K_train_HR/*.png')):
    ...
    y_in = cv2.resize(y_out, None, fx=0.25, fy=0.25, interpolation = cv2.INTER_AREA)
    x.append(y_in)
    y.append(y_out)
  x = np.array(x)
  y = np.array(y)
...
save_model_callback = tf.keras.callbacks.ModelCheckpoint(
    'drive/MyDrive/training/model/4x_upsample_model.h5',
    monitor = 'val_loss',
    verbose=1,
    save_best_only=True,
    mode='min',
...
tbCallBack = tf.keras.callbacks.TensorBoard(log_dir='./drive/MyDrive/training/graph/4x_upsample/', histogram_freq=0, write_graph=True, write_images=True)
...
train_model = load_model('drive/MyDrive/training/model/4x_upsample_model.h5')
img = cv2.imread("drive/MyDrive/training/test_image.png")

img_yuv = convert_RGB2YUV(img)
Y_downsampled, U_downsampled, V_downsampled = YUV_downsampling(img_yuv)
U_pred = np.expand_dims(U_downsampled,axis=0)
U_upsampled = train_model.predict(U_pred)

print("downsampled:",Y_downsampled.shape)
print("upsampled:",U_upsampled.shape)

plt.subplot(221)
plt.imshow(U_downsampled, cmap='gray')
plt.subplot(222)
